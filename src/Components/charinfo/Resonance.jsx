  ...
  return (
    <>
      <div className="page w-screen h-screen overflow-hidden relative flex flex-col items-center pt-12" id='resonance'>
        ...
        <img src={`/characters/${name}/main.png`} alt={`${name} main`} className={`charimg absolute ${isMobile ? 'bottom-[20vh] left-[20vw] w-[60vw]' : '-bottom-[10vw] left-0 w-[40vw]'}`} />

        {[...Array(6)].map((_, i) => (
          <img
            key={i}
            src={`/characters/${name}/resonance/${i + 1}.png`}
          ...
          {character.resoprior.map((index, key) => (
            <>
              <div className={`resopriorcont flex flex-col items-center h-full`}>
                <img
                  src={`/characters/${name}/resonance/${index}.png`}
