...
const Skills = ({ character, name }) => {
    ...
    if (!costToggle && hoveredIndex<6) {     
      return (
        ...
        <div className={`stitle flex items-center justify-between w-full ${isMobile?'h-[6vh] px-2':'px-12 h-[15%]'}`}>
          <div className="left flex items-center h-full">
            <img src={`${hoveredIndex===0?`/symbols/${character.weapon}.png`:`/characters/${name}/skills/${hoveredIndex}.png`}`} alt="" className="h-[110%] aspect-square object-cover" />
          ...
          {character.skilldes[hoveredIndex].map((i ,key) => (
            <div className={`flex flex-col items-center  ${isMobile?'w-[90%] gap-2':'h-full w-[44%]'}`}>
              <div className={`flex  items-center ${isMobile?'h-[2vh]':'h-[20vh]'}`}>
                <img src={`/characters/${name}/skills/${7-key}.png`} alt="" className={` object-cover ${isMobile?'h-[100%]':'h-[30%]'} ${hoveredIndex===7?'hidden':''}`}/>
      ...
      <div className={`slider-container h-[32vh] w-full bg-slate-30 rounded-t-[12vh] gap-3 rounded-md flex flex-col items-center`}>
        ...
        <div className={`flex flex-col items-center justify-start   ${isMobile?'h-[10vh] w-[15vw]':'h-[20vh] w-[18vh]'}`}>
            <img src={`/items/coin.png`} alt="" className='h-[65%]' />     
          ...
          <div className={`flex flex-col items-center justify-start   ${isMobile?'h-[10vh] w-[15vw]':'h-[20vh] w-[18vh]'}`}>
            <img src={`/items/${character.skillcost[val]}.png`} alt="" className='h-[65%]' />     
  ...
  return (
    <div className={`page overflow-hidden relative flex flex-col items-center pt-12 ${isMobile?'h-[120vh]':'h-[140vh]'}`} id="skills">
      ...
      <div className="">
          <img src={`/symbols/passive.png`} alt="" className={`h-[60%] imgskill object-cover ${6 === hoveredIndex ? 'scale-130' : ''}`} onMouseEnter={() => setHoveredIndex(6)} />
          <h2>Inherent Skills</h2>
        </div>
        <div className="">
          <img src={`/characters/${name}/skills/5.png`} alt="" className={`h-[60%] imgskill object-cover ${5 === hoveredIndex ? 'scale-130' : ''}`}
          onMouseEnter={() => setHoveredIndex(5)}/>
          <h2 className='text-white'>Outro Skills</h2>
        </div>
        <div className="">
          <img src={`/symbols/statbonus.png`} alt="" className={`h-[60%] imgskill object-cover ${7 === hoveredIndex ? 'scale-130' : ''}`}
          onMouseEnter={() => setHoveredIndex(7)}/>
          <h2 className='text-white'>Stat Bonus</h2>
        </div>
      </div>
      <div className={`togglebar  bg-slate-300 mb-2 rounded-full p-1 cursor-pointer  absolute  z-30 ${isMobile?'top-[73vh]  left-1/2 -translate-x-1/2':'bottom-[50vh] h-10 w-24 right-[11vw]'}`} onClick={Toggle}>
      ...
      <div className={`absolute aspect-square  skillwheel rounded-full ${isMobile ? 'h-[180vw] top-[90vh]' : 'h-[120vw] bottom-[-95vw]'} ${costToggle? 'rotate-180' : ''}`}>
        {character.skillprior.map((i, index) => (
          ...
          >
            ...
            <img
              src={`${i===0?`/symbols/${character.weapon}.png`:`/characters/${name}/skills/${i}.png`}`}
              alt={`${i + 1}`}
        ...
        <div className={`toggleskill  ${isMobile?'left-[50vw] bottom-[7vh]':'left-[30vw] bottom-[14vh]'}`}>
          <img src={`/symbols/passive.png`} alt="" className={`h-[80%] imgskill ${0 === costIndex? 'scale-130' : ''}`} onMouseEnter={() => handleCost(0)} />
          <h2>Inherent Skills</h2>
        </div>
        <div className={`toggleskill ${isMobile?'left-[84.5vw] bottom-[2vh]':'left-[54.5vw] bottom-[0vh]'}`}>
          <img src={`/symbols/forteall.png`} alt="" className={`h-[80%] imgskill ${1 === costIndex ? 'scale-130' : ''}`} onMouseEnter={() => handleCost(1)} />
          <h2>Forte Skills</h2>
        </div>
        <div className={`toggleskill ${isMobile?'left-[119vw] bottom-[7vh]':'left-[79vw] bottom-[14vh]'}`}>
          <img src={`/symbols/statbonus.png`} alt="" className={`h-[80%] imgskill ${2 === costIndex ? 'scale-130' : ''}`} onMouseEnter={() => handleCost(2)} />
          <h2>Stat Bonuses</h2>
        </div>
